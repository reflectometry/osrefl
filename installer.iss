; -- installer.iss -- an Inno Setup Script for OsRefl
; This script is used by the Inno Setup Compiler to build a Windows XP
; installer/uninstaller for the DANSE Reflectometry application named OsRefl.
...
; as "OsRefl 0.5.0", "OsRefl 0.7.2", and "OsRefl 1.0" under a group directory.
...
; By default, when installing OsRefl:
; - The destination folder will be "C:\Program Files\DANSE\osrefl x.y.z"
; - A desktop icon will be created with the label "OsRefl x.y.z"
; - A quickstart icon is optional
; - A start menu folder will be created with the name DANSE -> OsRefl x.y.z
; By default, when uninstalling OsRefl x.y.z
; - The uninstall can be initiated from either the:
;   * Start menu via DANSE -> OsRefl x.y.z -> Uninstall OsRefl
;   * Start menu via Control Panel - > Add or Remove Programs -> OsRefl x.y.z
; - It will not delete the C:\Program Files\DANSE\osrefl x.y.z folder if it
;   contains any user created files
; - It will delete any desktop or quickstart icons for OsRefl that were
...
#define MyAppName "OsRefl"
#define MyAppNameLowercase "osrefl"
...
#define Space " "
; Use updated version string if present in the include file.  It is expected that the OsRefl
